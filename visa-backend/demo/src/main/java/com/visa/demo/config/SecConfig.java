...
public class SecConfig {
    ...
    private final JAuthFilter jAuthFilter;
    private final UserDetailsService userDetailsService;

    public SecConfig(@Lazy JAuthFilter jAuthFilter, @Lazy UserDetailsService userDetailsService) {
        this.jAuthFilter = jAuthFilter;
        this.userDetailsService = userDetailsService;
    }

    @Bean
    public UserDetailsService userDetailsService(EmployeeDetailsService employeeDetailsService) {
        return employeeDetailsService;  // ✅ Explicitly defining UserDetailsService bean
    }
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/auth/login", "/error", "/favicon.ico","/api/employees/**").permitAll()
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        configuration.setAllowedOrigins(List.of("http://localhost:3000")); // ✅ Adjust frontend URL if needed
